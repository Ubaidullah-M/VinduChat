...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    @database_sync_to_async
    def create_message(self, message, id):
        return Message.objects.create(message=message, id=id, sender=self.chat.msg_sender chat=self.chat)
    ...
    if b"authorization" in self.scope["headers"]:
        ...
        if jwt_token:
            try:
                ...
                chat_pk = self.scope['url_route']['kwargs'].get('chat_id')
    ...
    async def receive(self, text_data):
        ...
        if not message or message.isspace():
            ...
            if self.chat.msg_receiver == sender or self.chat.msg_sender == sender:
            # Sender is authorized to send messages in this chat
                try:
                ...
                # Sender is not authorized to send messages in this chat
                await self.send(json.dumps({
                    'type': 'error',
                    'data': {

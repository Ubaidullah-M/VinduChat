...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    def create_message(self, message, id):
        ...
        if sender_id == str(self.chat.msg_sender.id) or sender_id == str(self.chat.msg_receiver.id):
            return Message.objects.create(message=message, id=id, sender_id=sender_id, chat=self.chat)
        else:
